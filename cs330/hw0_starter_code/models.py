## refer to 1. https://github.com/ChristianMontecchiani/CS-330-Deep-Multi-Task-and-Meta-Learning
##2. https://github.com/nguynking/CS330/blob/main/assignment0/code/models.py
...
class ScaledEmbedding(nn.Embedding):
    ...
    def reset_parameters(self):
        ...
        ## weights at padding_idx are initialized to 0 and not trained
...
class MultiTaskNet(nn.Module):
    ...
    def forward(self, user_ids, item_ids):
        ...
        predictions: tensor
            Tensoßr of user-item interaction predictions of shape (batch,), probability
        score: tensor
    ...
    def init_mlp_layers(self, layer_sizes):
        ...
        layer_sizes: list
            List of layer sizes (input_shape of each dense layers) to for the regression network.
        ...
        mlp_layers = mlp_layers.append(nn.Linear(layer_sizes[-1], 1))
        ### END CODE HERE ###
...
## The squeeze() operation is often useful when working with tensors that have redundant dimensions of size 1, as it can simplify the tensor's shape and make it easier to work with or perform certain operations.
## It's important to note that if the specified dimension does not have a size of 1, the squeeze() operation will not have any effect, and the tensor will remain unchanged.
    def forward_with_embedding_sharing(self, user_ids, item_ids):
        ...
        ### START CODE HERE ###
        u_i = self.U(user_ids) #(batch, embed_dim)
        q_j = self.Q(item_ids) #(batch, embed_dim)
        bias = self.B(item_ids).squeeze() #(batch, 1)
        # Regression head
        concat_embed = torch.cat((u_i, q_j, u_i*q_j),-1) # (batch, 3*embed_dim)
        score = self.mlp_layers(concat_embed).squeeze() #(batch, 1)

        # Matrix Factorization Head  
        predictions = ((u_i * q_j).sum(1) + bias).squeeze() #(batch, batch)
        # print('u*q',(u_i*q_j).sum(1).shape)
        # print('bias',bias.shape)
        # print('score',score.shape)
        # print('preditction', predictions.shape)
        ### END CODE HERE ###
        return predictions, score
    ...
    def forward_without_embedding_sharing(self, user_ids, item_ids):
        ...
        predictions = score = None
        ### START CODE HERE ###
        u_reg_i, u_fact_i = self.U_reg(user_ids), self.U_fact(user_ids)
        q_reg_j, q_fact_j = self.Q_reg(item_ids), self.Q_fact(item_ids)
        bias = self.B(item_ids).squeeze()

        # Regression head
        concat_embed = torch.cat((u_reg_i, q_reg_j, u_reg_i*q_reg_j),-1)
        score = self.mlp_layers(concat_embed).squeeze()

        # Matrix Factorization Head 
        predictions = ((u_fact_i * q_fact_j).sum(1) + bias).squeeze()
        ### END CODE HERE ###
        return predictions, score